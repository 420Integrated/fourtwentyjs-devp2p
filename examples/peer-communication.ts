...
import { FOURTWENTY, Peer } from '../src/index'
...
const rlpx = new devp2p.RLPx(PRIVATE_KEY, {
  dpt: dpt,
  maxPeers: 25,
  capabilities: [devp2p.FOURTWENTY.fourtwenty63, devp2p.FOURTWENTY.fourtwenty62],
...
rlpx.on('peer:added', peer => {
  ...
  const fourtwenty = peer.getProtocols()[0]
  const requests: {
    headers: any[]
    bodies: any[]
    msgTypes: { [key: string]: FOURTWENTY.MESSAGE_CODES }
  } = { headers: [], bodies: [], msgTypes: {} }

  const clientId = peer.getHelloMessage().clientId
  console.log(
    chalk.green(
      `Add peer: ${addr} ${clientId} (fourtwenty${fourtwenty.getVersion()}) (total: ${rlpx.getPeers().length})`,
    ),
  )

  fourtwenty.sendStatus({
    networkId: CHAIN_ID,
  ...
  fourtwenty.once('status', () => {
    fourtwenty.sendMessage(devp2p.FOURTWENTY.MESSAGE_CODES.GET_BLOCK_HEADERS, [CHECK_BLOCK_NR, 1, 0, 0])
    forkDrop = setTimeout(() => {
  ...
  fourtwenty.on('message', async (code: FOURTWENTY.MESSAGE_CODES, payload: any) => {
    if (code in FOURTWENTY.MESSAGE_CODES) {
    ...
    switch (code) {
      case devp2p.FOURTWENTY.MESSAGE_CODES.NEW_BLOCK_HASHES:
        ...
        for (let item of payload) {
          ...
          setTimeout(() => {
            fourtwenty.sendMessage(devp2p.FOURTWENTY.MESSAGE_CODES.GET_BLOCK_HEADERS, [blockHash, 1, 0, 0])
            requests.headers.push(blockHash)
          }, ms('0.1s'))
        }
        break

      case devp2p.FOURTWENTY.MESSAGE_CODES.TX:
        if (!forkVerified) break
      ...
      case devp2p.FOURTWENTY.MESSAGE_CODES.GET_BLOCK_HEADERS:
        const headers = []
        // hack
        ...
        if (requests.headers.length === 0 && requests.msgTypes[code] >= 8) {
          ...
          fourtwenty.sendMessage(devp2p.FOURTWENTY.MESSAGE_CODES.BLOCK_HEADERS, headers)
        }
        break

      case devp2p.FOURTWENTY.MESSAGE_CODES.BLOCK_HEADERS:
        if (!forkVerified) {
          if (payload.length !== 1) {
            console.log(
          ...
          while (requests.headers.length > 0) {
            ...
            if (header.hash().equals(blockHash)) {
              isValidPayload = true
              setTimeout(() => {
                fourtwenty.sendMessage(devp2p.FOURTWENTY.MESSAGE_CODES.GET_BLOCK_BODIES, [blockHash])
      ...
      case devp2p.FOURTWENTY.MESSAGE_CODES.GET_BLOCK_BODIES:
        if (requests.headers.length === 0 && requests.msgTypes[code] >= 8) {
          peer.disconnect(devp2p.DISCONNECT_REASONS.USELESS_PEER)
        } else {
          fourtwenty.sendMessage(devp2p.FOURTWENTY.MESSAGE_CODES.BLOCK_BODIES, [])
        }
        break

      case devp2p.FOURTWENTY.MESSAGE_CODES.BLOCK_BODIES:
        if (!forkVerified) break
      ...
      case devp2p.FOURTWENTY.MESSAGE_CODES.NEW_BLOCK:
        if (!forkVerified) break
      ...
      case devp2p.FOURTWENTY.MESSAGE_CODES.GET_NODE_DATA:
        if (requests.headers.length === 0 && requests.msgTypes[code] >= 8) {
          peer.disconnect(devp2p.DISCONNECT_REASONS.USELESS_PEER)
        } else {
          fourtwenty.sendMessage(devp2p.FOURTWENTY.MESSAGE_CODES.NODE_DATA, [])
        }
        break

      case devp2p.FOURTWENTY.MESSAGE_CODES.NODE_DATA:
        break

      case devp2p.FOURTWENTY.MESSAGE_CODES.GET_RECEIPTS:
        if (requests.headers.length === 0 && requests.msgTypes[code] >= 8) {
          peer.disconnect(devp2p.DISCONNECT_REASONS.USELESS_PEER)
        } else {
          fourtwenty.sendMessage(devp2p.FOURTWENTY.MESSAGE_CODES.RECEIPTS, [])
        }
        break

      case devp2p.FOURTWENTY.MESSAGE_CODES.RECEIPTS:
        break
    }
  })
})
...
// rlpx.listen(13013, '0.0.0.0')
// dpt.bind(13013, '0.0.0.0')
...
// connect to local 420coin node (debug)
/*
dpt.addPeer({ address: '127.0.0.1', udpPort: 13013, tcpPort: 13013 })
  .then((peer) => {
    return rlpx.connect({
      id: peer.id,
