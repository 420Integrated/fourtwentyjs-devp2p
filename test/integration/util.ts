...
import { DPT, FOURTWENTY, RLPx, genPrivateKey } from '../../src'
...
export function getTestRLPXs(numRLPXs: any, maxPeers: any, capabilities: any) {
  const rlpxs = []
  if (!capabilities) {
    capabilities = [FOURTWENTY.fourtwenty63, FOURTWENTY.fourtwenty62]
  }
...
export function twoPeerMsgExchange(t: Test, capabilities: any, opts: any) {
  ...
  rlpxs[1].on('peer:added', function(peer: any) {
    ...
    protocol.on('message', async (code: any, payload: any) => {
      switch (code) {
        // Comfortability hack, use constants like devp2p.FOURTWENTY.MESSAGE_CODES.STATUS
        // in production use
        case 0x00: // (-> 1)
